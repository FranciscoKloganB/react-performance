...
In this lesson we learned that state collocation is not only important for long
term project maintainability, but can also impact performance, by helping to
avoid unnecessary re-renders, as side-effect of changes on other parts of the
tree.

We first collocated state, than did that while still granding somewhat global
access to a DogContext, if that was required for the application to work.

Then, we decided to tackle problems on the grid. We noticed, that when clicking
one element (`<row, column>`) all other elements were re-rendering. So first,
we decided to create a component that was responsible for consuming app state
changes and passing the new values down to an implementation of the actual
component. The benefit here was that we were able to memoize `CellImpl`
components, because `Cell` only job became consuming the state and passing it
down to the actual implementation of `Cell`, `CellImpl`. This is called state
slicing.

On the next exercise, we created a generic, `High Order Component`, that
granted generic way to slice state. Avoiding the need to create multiple
`<Comp, CompImpl>` pairs for different components, if that they were ever
needed.

Lastly, we learned that most of the time this optimizations are not necessary
and that alternatives such as state management with `Recoil.js`, an official
React alternative to `Redux`, might be a better choice. Recoil has a simpler
API than Redux, and has the same philosophy and approach to code as React.
While providing this kind o state optimizations out of the box. Notice that,
we still should avoid using global state everywhere. :)
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Performance%20%E2%9A%A1&e=06%3A%20Fix%20%22perf%20death%20by%20a%20thousand%20cuts%22&em=francisco.teixeira.de.barros%40outlook.com).
