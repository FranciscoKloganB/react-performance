...
In this module we learn how to perform the easiest possible kind of optimization
code splitting. Code splitting is a technic that involves loading components and
scripts asynchronously to ensure that the first time to interaction from the user
is decreased.

Depending on the situation and what we desire the user experience to be, also
depending on his usage (mobile vs desktop, low connectivity vs fiber connection)
we can: Lazy Load, Eager Load or Prefetch components.

* Lazy Loading an item postpones asynchronous download until the user actually
requires the component, e.g.: accesses a DOM element that requires that item.

* Eager Loading ressembles Lazy Loading, but differs from it because it
optimistically attempts to start downloading the item, before the user actually
requires the item, e.g.: when he hovers over a DOM element which, once clicked
would make the user require the missing item.

* Prefetch is a form of an asynchronous loading, which avoids startup slowdown
but starts downloads as soon as the browser has the opportunity to execute the
download. In a sense, it provides the fastest experience of the three, but
increases the chance that the user downloads code or assets he does not need,
which can be a problem if he is for example, using mobile data.

As a bonus. We also learned that when we are using any of these techniques
over React components, we need to define boundaries with `React.Suspense`
which receives a `fallback` prop, which is rendered while the loading of that
item is being carried out.

Finally, we must remember that, we should code first, optimize later. Optimization
should be done with care, always based on the production code that is served
to the client and based not only on `network` tab, but also on the `coverage` tab
and the `React Profiler` and any other available metric.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Performance%20%E2%9A%A1&e=01%3A%20Code%20splitting&em=francisco.teixeira.de.barros%40outlook.com).
