...
In this lesson, we used React Memo to avoid unnessary re-rendering over components
whose props or derived data does not change. While React Memo is straighforward
to use, it still is not perfect. For example, on this exercise, when we hovered
over a list item, all list items were re-rendered, when only two of them needed
to be re-rendered (the newly highlighted item and the previously highlited) item.

When we learned about this issue, we used a custom comparison function, that helps
React Memo to decide wether or not to re-render each item on the List.

Alas, things seem better. The problem is that, this custom comparison functions
are not always easy to implement and sometimes they are not cost-effective in
terms of development investment. They can also be the root cause of bugs that
are hard to find, consequently, the custom comparison function, passed as second
argument, should be avoided unless stricly necessary.

On the final extra credit, we realized, that, React Memo default implementation
deals well with primitive type comparisons. Therefore, if we could find away
to pass `isHighlighted` and `isSelected` as props to a ListItem, instead of
calculating on each `ListItem` if it is selected or highlighted (or not) based,
for example, on a given `"selectedIndex"` we could get the same performance
improvement without all the complications.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Performance%20%E2%9A%A1&e=03%3A%20React.memo%20for%20reducing%20unnecessary%20re-renders&em=francisco.teixeira.de.barros%40outlook.com).
