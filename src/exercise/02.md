...
In this class we learned how to reduce the number of times we perform expensive
calculations using `useMemo`. We also learned about an alternative method using
web workers. Web workers run jobs on seperate threads managed by the browser,
which are capable of communicating the results asynchronously back to the main
javascript thread.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Performance%20%E2%9A%A1&e=02%3A%20useMemo%20for%20expensive%20calculations&em=francisco.teixeira.de.barros%40outlook.com).
